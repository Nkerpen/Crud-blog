...
<div class="container" class="d-flex justify-content-center">
    <div id="lista-noticias-n" class="mt-5">
        <table class="table table-striped table-bordered border-2 table-hover">
            ...
            $query = mysqli_query($conexao, "SELECT * FROM NOTICIAS INNER JOIN imgs ON noticiaImgId = imgId INNER JOIN infos ON noticiaInfoId = infoId INNER JOIN usuarios ON noticiaUsuarioId = usuarioId where noticiaId = $varPostagemCodigo");
            while ($exibe = mysqli_fetch_array($query)) {
                ...
                <?php
                $queryDois = mysqli_query($conexao, "SELECT * FROM NOTICIAS INNER JOIN imgs ON noticiaImgId = imgId INNER JOIN infos ON noticiaInfoId = infoId INNER JOIN usuarios ON noticiaUsuarioId = usuarioId where noticiaUsuarioId = $exibe[3] and noticiaId != $exibe[0]");
